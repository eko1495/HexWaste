...
- **WAIT / FOLLOW toggle:** no separate "wait" boolean — the FOLLOW-OPTIONS menu (Sulik Node1007 @0xa746; Vic ends @0x508c) just writes the distance LVAR to different values; "wait here" = script ceases chasing because `GVAR[398]`/`LVAR[11]` is set. Our heartbeat **must consult GVAR398 + the per-script wait LVAR** (both default 0 = follow). Menu strings load via `message_str(file=14, id=10004..10013)` = the shared **partymbr.msg-class** file, NOT kcsulik.msg.
  > **CORRECTED (#8, 2026-06):** the `message_str(14,…)` call in the disassembly may be
  > real, but the "= partymbr.msg" identification was an unconfirmed guess and is FALSE
  > for this data. `partymbr.msg` does not exist in the slice, "partymbr" is absent from
  > the fallout2-ce source, and message list 14 resolves to `generic.msg` here (not a
  > party file). Moot in any case: Sulik/Vic are not recruitable in the shipped slice,
  > and the viewer hub uses English fallback labels with no `message_str(14,…)` lookup.
...
No quest XP/karma in this slice (team kills already pay XP, p7-m4, unaffected). ~~Soft blocker for the follow-options menu text: resolve **message_str file id 14** (partymbr.msg-class) or option strings render blank.~~ **CORRECTED (#8):** not a blocker — partymbr.msg/list-14 does not exist (see line 101 note); the shipped hub uses English fallback labels, no list-14 lookup.
