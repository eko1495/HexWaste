...
- **Inventory** (`DrawItemPanels`/`DrawInventoryWindow`/`DrawItemWindow`/`DrawEquipSlots`/
  `DrawInventorySummary`, `ViewerGame.Panels.cs`)
...
- **Inventory** — origin helpers `InvBoxOrigin()` (`ViewerGame.Panels.cs:1819`) and
  `ItemWindowArt()` (`:1875`, used for the loot/barter/trade window variants). Every caller of
  both is inside `ViewerGame.Panels.cs`, all part of Inventory's own screen logic (equip slots,
  row rects, the Done button, drag-and-drop). Mouse hit-testing happens only in `Update()`
  (`ViewerGame.cs`), reading the raw per-frame `mouse` local at four sites: `:2453`
  (`InvBoxDoneRect().Contains`), `:2460` (`LootDoneRect().Contains`), `:2466`
  (`HandleInventoryDrag(mouse, ...)`), `:2468` (`TryClickItemPanel(mouse.X, mouse.Y, ...)`).
  Inventory has no separate art-missing text-fallback branch — its content (item icons, equip
  slots) is drawn directly, not gated behind an "art loaded?" boolean the way the main-menu
  family was.
...
For each of the four screens, apply the same technique Stage 2 established:

1. **Scope a `SpriteBatch` block around the screen's own `Draw*` call(s).** Following the
   ...
   `DrawAuthenticMainMenu`'s `bool` return already gates today.
2. **Switch every named origin helper's viewport read from `GraphicsDevice.Viewport(.Bounds)` to
   `VirtualViewport()`** (Stage 1, `ViewerGame.UiScale.cs`): `InvBoxOrigin()`, `ItemWindowArt()`,
   `SkilldexOrigin()`, `PerkWindowOrigin()`, `SaveLoadPanelRect()`. Do NOT touch
   ...
   reads) — those stay on the raw device viewport, unscaled, per point 1.
3. **Switch every mouse read used for position (not button state) inside a scaled block to
   `UiMouse()`**: the four Draw-side hover reads (Inventory has none; Skilldex `:649`, Perk picker
   `:551`, Save/Load `:1347`) and the `Update()`-side hit-tests (Inventory's four sites, Skilldex's
   one, Perk picker's one, Save/Load's one) — reusing the existing `uiMouse` local for the
   `Update()` sites, and a fresh `UiMouse()` call for the Draw-side hover reads (matching how
   `DrawConversationPanel`'s hover loop already does this in Stage 2).
...
- Any change to `ViewerGame.Harness.cs` — none of Stage 3a's four screens has a harness caller of
...
main-menu family, centered, undistorted, with hover/click still landing under the cursor. For
Skilldex and Perk picker specifically, also confirm the (headless-only-reachable-live-when-art-
missing) text fallback still renders at native size, unscaled — matching the Stage 2 precedent
this spec's design section calls for.
