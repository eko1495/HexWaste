...
SCENARIOS=(
  ...
  # P1-M2: the recurring 600-tick map_update heartbeat (scripts.cc:517 mapUpdateEventProcess). Deterministic
  # cadence — half an interval fires 0, each full 600-tick interval fires exactly once (beats=3 -> 3 fires).
  "map-update-heartbeat|--map denbus2.map --map-update-beats 3 --rng-seed 1"
  ...
  # P100 (Point 4): the optional-authenticity read-outs. car-probe exercises the ported Highwayman fuel
  # model; quest/holodisk probes prove the Pip-Boy Archives populate from set_global_var (forced here).
  "car-model|--map artemple.map --car-probe --rng-seed 1"
  # P100 (bucket 1): the car GAMEPLAY payoff — driving covers stride>1 pixels/step + burns 100 fuel/step;
  # low fuel strands the party on cardesrt (out-of-gas). Foot travel (InCar=false) is byte-identical.
  "car-travel|--car-acquire 184 133 1 --rng-seed 2"
  "car-outofgas|--car-acquire 184 133 1 500 --rng-seed 2"
  "quest-geck-done|--map artemple.map --set-global 480 5 --quest-probe --rng-seed 1"
  "holodisk-220|--map artemple.map --set-global 220 1 --holodisk-probe --rng-seed 1"
  ...
  # P78-M2 — enemy combat-drug use: give Metzger (denbus2 @15278) a Buffout (pid 87) and run the real
  # TryNpcUseCombatDrug; the immediate SPECIAL buff (ST+2/EN+3/AG+2) folds into his CritterState. The
  # decision (chem_use chance) is fake-host tested; this probe locks the per-NPC buff on real data.
  "ai-drug|--map denbus2.map --ai-drug-probe 15278 87 --rng-seed 1"
