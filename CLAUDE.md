...
**The full phase-by-phase history (Phase 1 → 81 + backlog + maintenance) lives in
[`docs/PHASE-HISTORY.md`](docs/PHASE-HISTORY.md)** — moved out to keep this file under
the context-load limit. Read it when you need the rationale, gotchas, or `P<n>-M<m>`
cross-references behind a shipped feature.

### Phase status summary

- **Phase 1–9** (DONE): core DAT2/FRM/PAL/MAP/PRO pipeline, rendering, palette
  cycling, then the "walking simulator" + combat depth (critters, animation,
  picking, movement, interactions, dialog, doors, combat engine extraction).
- **Phase 10–53** (DONE): the full single-slice game loop — worldmap travel +
  encounters, party/companions, character progression (traits/perks/skills),
  stealth, karma, combat fidelity (crits, bursts, AP, AI), the authentic HUD +
  UI panels, save/load, sfx, and presentation polish.
- **Phase 54–66** (DONE): every original-game location wired — Vault City,
  Gecko, Modoc, Broken Hills, New Reno, NCR, San Francisco, Redding, Vault 15,
  Sierra, Military Base, Navarro, the Enclave Oil Rig. The entire map set loads,
  walks, transitions, and runs its scripts; remaining gaps are content, not engine.
- **Phase 67–81** (DONE): paperdoll inventory, AI/perk/stat fidelity passes,
  faithful automap fog, float messages, NPC-vs-NPC brawls, AP-dodge, dual-wield.
