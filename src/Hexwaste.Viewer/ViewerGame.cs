...
public sealed partial class ViewerGame : Game, Formats.Combat.ICombatHost
{
    ...
    public abstract record StartupAction
    {
        ...
        // P101 (bucket 1c): load a per-head .lip from the DAT + print the phoneme→frame chain at sample marks.
        public sealed record LipProbe(string Head, string Audio) : StartupAction;
    ...
    private bool ScriptCanSee(MapObject source, MapObject target)
    {
        ...
        // predicate (SightBlockerAt), so no SHOOT_THRU/NO_BLOCK term applies. The filter only
